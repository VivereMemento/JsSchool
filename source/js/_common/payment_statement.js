    ...
    return data.reduce((obj, key) => {
        if (obj.hasOwnProperty(key[value])) {
            ...
            obj[key[value]] = {
                ...
                totalPayment: 0,
                getAvgPayment() {
                    return this.totalPayment / this.quantity;
                }
            };
            return obj;
        }
...
const getFremeWithTotalPayment = (frame, value) => {
    return data.reduce((state, obj) => {
        return Object.keys(state).reduce((nextState, val) => {
            if (obj[value] === val) {
                state[val].totalPayment += obj.amount;
            }
            return state;
        }, {})
    }, frame);
...
const getTotalPayment = data.reduce((sum, num) => {
    return parseFloat(sum.toFixed(2)) + num.amount;
}, 0);

const getAvgPayment = (obj, method) => {
    const data = {};
    for (let key in obj) {
        data[key] = method[key].getAvgPayment().toFixed(2);
    }
    return data;
};

const createTable = (selecContainer, headCells, data) => {
...
const totalPaymentOfDep = getFremeWithTotalPayment(departments, 'department');
const avgPaymentOfDep = getAvgPayment(departments, totalPaymentOfDep)
const months = getFrame('month');
const totalPaymentOfMonths = getFremeWithTotalPayment(months, 'month');
const avgPaymentOfMonths = getAvgPayment(months, totalPaymentOfMonths)
...
totalTable.innerText = getTotalPayment;
